...
_usage() {
    ...
    Usage:
        ...
        -u  - adds username@$DOMAIN to mailserver and carddav with given password.
...
case $1 in
    "init")
        ...
        firewall-cmd --reload
        # TODO: Add email firewall rules here

        # Get docker-mailserver's setup tool
        if [[ ! -f "./data/mail/mail.sh" ]]; then
